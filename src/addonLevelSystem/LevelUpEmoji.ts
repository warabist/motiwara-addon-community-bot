...
export class LevelUpEmoji {
  ...
  register(client: Client, userdb: Database): void {
    try {
      ...
      console.log(this.emojiName + 'としてレベルアップ絵文字を登録しました');
    } catch (err) {
      console.error(this.emojiName + 'のレベルアップ絵文字登録を失敗しました');
      throw err;
    }
  }
  ...
  private async handleReactionChange(
    ...
    try {
      ...
      this.adjustLevel(resolvedMessage.author, this.levelUpAmount, thread);
  ...
  /** レベル管理処理を飛ばすかを返す */
  private shouldSkipProcessing(reaction: MessageReaction, user: User): boolean {
    ...
    return isBot || isNotLevelUpEmoji || isNotQuestionChannelParent || isOwner;
  }
}
